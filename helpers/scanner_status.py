...
from helpers.scanner_manager import set_new_last_scanned_date, set_quest_scanning_state, rename_voice_channel, start_pokestop_scan, clear_old_pokestops_gyms, restart_run_docker_containers
from helpers.utilities import time_now, clear_quest_file, build_embed_object_title_description, log_to_file, build_embed_object_title_description, did_day_change, private_message_user_by_id, run_database_query
...
async def check_map_status():
    ...
    pokemonScanResults = run_database_query("SELECT pokestop_id FROM pokestop WHERE last_updated > NOW() - INTERVAL 30 MINUTE ORDER BY last_updated DESC LIMIT 1;")
...
async def is_quest_scanning():
    didDayChangeFromStoredDb = run_database_query(f"SELECT last_scanned_date FROM poliswag WHERE last_scanned_date < '{time_now()}' OR last_scanned_date IS NULL;", "poliswag")
    if len(str(didDayChangeFromStoredDb).split("\\n")) > 1:
        lastScannedDate = str(didDayChangeFromStoredDb).split("\\n")[1]
...
def verify_quest_scan_done():
    totalPreviousScannedStops = run_database_query(f"SELECT SUM(pokestop_total_leiria) AS total_sum FROM poliswag;", "poliswag")
    if len(str(totalPreviousScannedStops).split("\\n"))> 1:
        totalScannedStops = str(totalPreviousScannedStops).split("\\n")[1]
        with open(constants.QUESTS_FILE) as raw_data:
            jsonPokemonData = json.load(raw_data)
        return len(jsonPokemonData) >= totalScannedStops
    return False
