...
class RoleManager:
    async def prepare_view_roles_location(self, channel):
        buttonLeiria = Button(
            label="POKÉMON LEIRIA",
            style=discord.ButtonStyle.secondary,
            custom_id="AlertasLeiria",
            row=0,
        )
        buttonMarinha = Button(
            label="POKÉMON MARINHA",
            style=discord.ButtonStyle.secondary,
            custom_id="AlertasMarinha",
            row=0,
        )
        buttonRaidsLeiria = Button(
            label="@LEIRIA",
            style=discord.ButtonStyle.secondary,
            custom_id="Leiria",
            row=2,
        )
        buttonRaidsMarinha = Button(
            label="@MARINHA",
            style=discord.ButtonStyle.secondary,
            custom_id="Marinha",
            row=2,
        ...
        buttonRaids = Button(
            label="ALERTAS RAIDS",
            style=discord.ButtonStyle.secondary,
            custom_id="AlertasRaids",
            row=1,
        )
        buttonPvP = Button(
            label="ALERTAS PvP",
            style=discord.ButtonStyle.secondary,
            custom_id="AlertasPvP",
            row=1,
        )
        ...
        embed = discord.Embed(
            title="NOTIFICAÇÕES DE POKÉMON/RAIDS",
            description="Para desativar ou ativar as notificações, basta clicar no botão correspondente.\n",
            color=0x7B83B4,
        )
        embed.add_field(
            name="POKÉMON LEIRIA/MARINHA",
            value="Receber notificações do aparecimento dos pokémon no centro de Leiria e/ou Marinha Grande de diferentes IVs;",
            inline=False,
        )
        embed.add_field(
            name="ALERTAS RAIDS/PvP",
            value="Receber notificações de Raids e/ou de pokémon ideais para PvP;",
            inline=False,
        )
        embed.add_field(
            name="@LEIRIA/@MARINHA/@REMOTE",
            value="Receber menções quando identificam o role Leiria, Marinha ou remote;",
            inline=False,
        )
        await channel.send(embed=embed, view=view)

    async def prepare_view_roles_teams(self, channel):
        buttonInstinct = Button(
            label="INSTINCT",
            style=discord.ButtonStyle.secondary,
            custom_id="Instinct",
            emoji="<:instinct:596737766038175775>",
        )
        buttonMystic = Button(
            label="MYSTIC",
            style=discord.ButtonStyle.secondary,
            custom_id="Mystic",
            emoji="<:mystic:596737698056634389>",
        )
        buttonValor = Button(
            label="VALOR",
            style=discord.ButtonStyle.secondary,
            custom_id="Valor",
            emoji="<:valor:596737734920503314>",
        )
        ...
        embed = discord.Embed(
            title="SELEÇÃO DE EQUIPA",
            description="Mais uma vez, sejam bem vindos ao discord **PoGoLeiria**, para começares e desbloqueares os restantes canais, pressiona no botão correspondente à tua equipa no jogo.\nPara as **notificações dos spawns de Pokémon/Raids**, podem-nas ativar em <#985883269193146408>",
            color=0x7B83B4,
        )
        await channel.send(embed=embed, view=view)
    ...
    def build_response_message(self, role):
        if (
            role.lower() == "alertasleiria"
            or role.lower() == "alertasmarinha"
            or role.lower() == "alertaspvp"
            or role.lower() == "alertasraids"
            or role.lower() == "remote"
        ):
            return "Notificações de " + role.title()
        elif (
            role.lower() == "mystic"
            or role.lower() == "valor"
            or role.lower() == "instinct"
        ):
    ...
    async def build_rules_message(self, message):
        ...
        embed.add_field(
            name="Nada de conteúdo pornográfico, odioso, prejudicial e/ou ofensioso",
            value="Todo conteúdo que encaixe nestes moldes, sofrera de tolerância zero e será alvo de moderação, sendo avatares também abrangidos por estas regras;",
            inline=False,
        )
        embed.add_field(
            name="Proibido spam e cross channel spam",
            value="Qualquer canal de discussão pública não deve ser usado para qualquer tipo de publicidade e/ou spam/cross channel spam;",
            inline=False,
        )
        embed.add_field(
            name="Proibida conversa sobre troca e venda de contas",
            value="De modo a tornar o jogo mais justo, toda e qualquer conversa relacionada com a compra/venda de contas e/ou troca/partilha de contas, é proibida;",
            inline=False,
        )
        embed.add_field(
            name="Botting e GPS Spoofing;",
            value="Nada de discussão ou conversas relacionadas com ações de botting ou GPS spoofing que prejudicam outros jogadores;",
            inline=False,
        )
        embed.add_field(
            name="A administração poderá moderar à sua discrição",
            value="Sendo impossísvel prever todas as situações, a equipa de administração poderá moderar qualquer assunto que considerem impróprios. Como tal, bom senso nas interações com os restantes membros da comunidade.",
            inline=False,
        )
        ...
        if isinstance(user, discord.Member):
            for role in user.roles:
                try:
                    ...
                    print(
                        f"Bot doesn't have permission to manage roles when removing roles from {user}"
                    )
        else:
