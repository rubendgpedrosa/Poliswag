            ...
            if event["start"] and event["end"]:
                ...
                if not unannounced:
                    self.poliswag.db.execute_query_to_database(
                        f"INSERT IGNORE INTO event(name, start, end) VALUES ('{event_name_escaped}', '{event['start']}', '{event['end']}');"
                    )

    def get_current_active_events(self):
        current_time = datetime.now()
        stored_events = self.poliswag.db.get_data_from_database(
            f"SELECT name, start, end, notification_date FROM event WHERE (notification_date IS NULL AND start < '{current_time}') OR (notification_date < end AND end < '{current_time}');"
        )
        if len(stored_events) == 0:
            return

        if isinstance(stored_events, list):
            names_stored_events = ", ".join(
        ...
        self.poliswag.db.execute_query_to_database(
            f"UPDATE event SET notification_date = CASE WHEN notification_date IS NULL THEN start ELSE end END WHERE name IN({names_stored_events});"
        )
