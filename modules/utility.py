...
class Utility:
    ...
    async def get_new_pokemongo_version(self):
        ...
        if response.status_code == 200:
            ...
            stored_version = self.poliswag.db.get_data_from_database(
                f"SELECT version FROM poliswag"
            )
            if retrieved_version != stored_version["version"]:
                self.poliswag.db.execute_query_to_database(
                    f"UPDATE poliswag SET version = '{retrieved_version}'"
                )
        ...
        if log_type == "ERROR":
            with open(self.LOG_FILE, "a") as file:
                file.write(
                    log_type
                    + " | {0} -- {1}\n".format(
                        datetime.now().strftime("%Y-%m-%d %H:%M"), string
                    )
                )
        elif log_type == "CRASH":
    ...
    def time_now(self):
        ...
        )  # create a time object with 00:00:00
