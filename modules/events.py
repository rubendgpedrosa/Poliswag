    ...
    for event in events:
        if event["start"] is not None:
            ...
            if (
                isBeforeStartDate
                and hasQuests
                and isNotCommunityDay
                and isNotGoEvent
                and isNotUnannounced
                and isNotSpotlightHour
                and isNotRaidHour
            ):
                eventNameEscaped = event["name"].replace("'", "\\'")
                insertedRows = execute_query_to_database(
                    f"INSERT IGNORE INTO event(name, start, end, has_quests, has_spawnpoints, rescan) VALUES ('{eventNameEscaped}', '{event['start']}', '{event['end']}', {int(hasQuests)}, {int(hasSpawnpoins)}, 1);",
                    "poliswag",
                )
                if insertedRows < 0:
                    await add_scheduled_event_discord_sidebar(
...
async def ask_if_automatic_rescan_is_to_cancel():
    ...
    if len(eventsByStartTime) > 0:
        ...
        for start_time, event_dict in eventsByStartTime.items():
            ...
            buttonCancelRescan = Button(
                label="CANCELAR",
                style=discord.ButtonStyle.danger,
                custom_id=str(start_time),
                row=1,
            )
            ...
            execute_query_to_database(
                f"UPDATE event SET notifieddate = NOW() WHERE start = '{start_time}';",
                "poliswag",
            )


def get_events_stored_in_database_to_rescan():
    storedEvents = get_data_from_database(
        "SELECT name, start FROM event WHERE start > NOW() AND start < NOW() + INTERVAL 36 HOUR AND notifieddate IS NULL;",
        "poliswag",
    )
    eventsDict = {}
...
async def cancel_rescan_callback(interaction: discord.Interaction):
    try:
        ...
        embed = discord.Embed(
            title=f"RESCAN CANCELADO",
            description=f"{interaction.user} cancelou rescan automático!",
            color=0x7B83B4,
        )
...
async def initialize_scheduled_rescanning_of_quests():
    isQuestScanningScheduled = get_data_from_database(
        "SELECT name FROM event WHERE rescan = 1 AND ((NOW() BETWEEN start AND DATE_ADD(start, INTERVAL 15 MINUTE) AND TIME(start) != '00:00:00') OR (NOW() BETWEEN end AND DATE_ADD(end, INTERVAL 15 MINUTE) AND TIME(end) != '00:00:00'));",
        "poliswag",
    )
    if len(isQuestScanningScheduled) > 0:
        questScannerRunning = get_data_from_database(
            "SELECT scanned FROM poliswag WHERE scanned = 0;", "poliswag"
        )
        if len(questScannerRunning) > 0:
            ...
            execute_query_to_database(
                "UPDATE event SET rescan = 0 WHERE notifieddate IS NULL AND NOW() > DATE_SUB(start, INTERVAL 1 DAY);",
                "poliswag",
            )
...
async def set_if_to_rescan_on_event_start(date, rescan=0):
    execute_query_to_database(
        f"UPDATE event SET rescan = '{rescan}', updateddate = NOW() WHERE start = '{date}';",
        "poliswag",
    )
...
async def notify_quest_channel_scan_start():
    ...
    await questChannel.send(
        embed=build_embed_object_title_description(
            "Alteração nas quests detetada", "Novo scan inicializado!"
        )
    )
...
async def notify_event_bonus_activated():
    ...
    for event in events:
        ...
        if event["start"] is None or event["end"] is None:
            continue
        start = datetime.strptime(event["start"], "%Y-%m-%d %H:%M").replace(
            second=0, microsecond=0
        )
        end = datetime.strptime(event["end"], "%Y-%m-%d %H:%M").replace(
            second=0, microsecond=0
        )
    ...
    if neverNotified:
        await convivioChannel.send(
            embed=build_embed_object_title_description(
                "Paaaaauuuuuuuu...??", "Acabaram-se os eventos :("
            )
        )
...
async def retrieve_database_upcoming_events(message):
    upcomingEvents = get_data_from_database(
        "SELECT name, start, end FROM event WHERE rescan = 1 AND start > NOW() ORDER BY start ASC;",
        "poliswag",
    )
    if len(upcomingEvents) > 0:
        messageToSend = build_embed_object_title_description(
            "Próximos Quest rescan agendados"
        )
        for event in upcomingEvents:
            messageToSend.add_field(
                name=event["data"][0],
                value=f"De {event['data'][1]} até {event['data'][2]}",
                inline=False,
            )
    else:
