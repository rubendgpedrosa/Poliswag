...
class ScannerStatus:
        ...
        if workerStatus and 'areas' in workerStatus:
            for area in workerStatus['areas']:
                ...
                if 'worker_managers' in area and area['worker_managers']:
                    expectedWorkers = area['worker_managers'][0].get('expected_workers', 0)

                downDevices = expectedWorkers
                print(f"Area: {area.get('name')}, Expected Workers: {expectedWorkers}")  # Log expected workers
                ...
                for workerManager in area.get('worker_managers', []):
                    for worker in workerManager.get('workers', []):
                        ...
                        print(f"Connection Status: {connection_status}")  # Log connection status

                        isWorkerUp = False

                        if last_data is not None:
                            time_difference = datetime.datetime.now().timestamp() - last_data
                            print(f"Time Difference: {time_difference}")  # Log time difference
                            if time_difference <= 300:  # 5-minute threshold
                                if connection_status == "Executing Worker":
                                    ...
                                    print(f"Unknown connection status: {connection_status}")  # Log unknown status
                            else:
                ...
                if area.get('name') == "LeiriaBigger":
                    downDevicesLeiria = max(downDevices, 0) if downDevices is not None else None
                elif area.get('name') == "MarinhaGrande":
                    downDevicesMarinha = max(downDevices, 0) if downDevices is not None else None

                print(f"Area: {area.get('name')}, Down Devices: {downDevices}")  # Log down devices for the area

        print({'downDevicesLeiria': downDevicesLeiria, 'downDevicesMarinha': downDevicesMarinha})  # Log final counts
        return {'downDevicesLeiria': downDevicesLeiria, 'downDevicesMarinha': downDevicesMarinha}
    ...
    async def is_quest_scanning_complete(self):
        questScanningOngoing = self.poliswag.db.get_data_from_database(f"SELECT scanned FROM poliswag;")
        ...
        try:
            ...
            return {'leiriaCompleted': leiriaCompleted, 'marinhaCompleted': marinhaCompleted}
    ...
    async def get_account_stats(self):
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(self.SCANNER_ACCOUNTS_STATUS_URL, timeout=20) as response:
                    response.raise_for_status()
