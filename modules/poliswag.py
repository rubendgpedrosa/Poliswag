...
from modules.utility import log_to_file, build_embed_object_title_description
from modules.database_connector import get_data_from_database, execute_query_to_database
...
def load_filter_data():
    ...
    for name in namesList:
        ...
        embed.add_field(
            name=discordMessageChannels[name], value=discordMessage, inline=False
        )
...
def fetch_new_pvp_data():
    ...
    build_notification_data_for_pvp(
        greatLeagueData, ultraLeagueData, jsonFiltersPokemonData
    )
...
def fetch_data_from_endpoint(combat_power):
    request = requests.get(
        f"https://raw.githubusercontent.com/pvpoke/pvpoke/master/src/data/rankings/all/overall/rankings-{combat_power}.json"
    )
...
def prepare_fetched_json_object(jsonData):
    ...
    jsonDataNoDuplicates = [
        k for j, k in enumerate(jsonData) if k not in jsonData[j + 1 :]
    ]
...
def build_notification_data_for_pvp(
    greatLeagueData, ultraLeagueData, jsonFiltersPokemonData
):
    ...
    for jsonPokemonListX in jsonPokemonList:
        ...
        for jsonPokemonListY in jsonPokemonListX:
            ...
            if (
                pokemonName in greatLeagueData
                and pokemonName
                not in jsonFiltersPokemonData["monsters"]["filters"]["pvp_great"][
                    "monsters"
                ]
            ):
                for nameToInsert in familyList:
                    jsonFiltersPokemonData["monsters"]["filters"]["pvp_great"][
                        "monsters"
                    ].append(nameToInsert)
                    jsonFiltersPokemonData["monsters"]["filters"]["pvp_great_marinha"][
                        "monsters"
                    ].append(nameToInsert)
            if (
                pokemonName in ultraLeagueData
                and pokemonName
                not in jsonFiltersPokemonData["monsters"]["filters"]["pvp_ultra"][
                    "monsters"
                ]
            ):
                for nameToInsert in familyList:
                    jsonFiltersPokemonData["monsters"]["filters"]["pvp_ultra"][
                        "monsters"
                    ].append(nameToInsert)
                    jsonFiltersPokemonData["monsters"]["filters"]["pvp_ultra_marinha"][
                        "monsters"
                    ].append(nameToInsert)


async def build_commands_message(message):
    embed = discord.Embed(title="COMANDOS IMPLEMENTADOS", color=0x7B83B4)

    embed.add_field(
        name="!alertas", value="Lista de Pokémon nas Notificações", inline=False
    )
    embed.add_field(
        name="!add <pokemon> <canal>",
        value="Adicionar pokémon a canal de notificações",
        inline=False,
    )
    embed.add_field(
        name="!remove <pokemon> <canal>",
        value="Remover pokémon de canal de notificações",
        inline=False,
    )
    embed.add_field(
        name="!reload", value="Submeter alterações nas notificações", inline=False
    )
    ...
    embed.add_field(
        name="!lures", value="Lista as contas com lures disponíveis", inline=False
    )
    embed.add_field(
        name="!uselure <uername> <number>",
        value="<number> negativo -> remove <number> lures.\n<number> positivo -> adiciona <number> lures.",
        inline=False,
    )
    embed.add_field(
        name="!upcoming", value="Lista rescan de quests agendados", inline=False
    )
    embed.add_field(
        name="!track <reward>",
        value="Adiciona reward a sumário de quests",
        inline=False,
    )
    embed.add_field(
        name="!untrack <reward>",
        value="Remove reward de sumário de quests",
        inline=False,
    )
    embed.add_field(
        name="!tracklist", value="Lista rewards flagged como importantes", inline=False
    )
    embed.add_field(
        name="!untrackall", value="Limpa toda a lista de rewards", inline=False
    )
...
def get_available_pogo_accounts():
    ...
    storedAvailableAccounts = get_data_from_database(
        f"SELECT username FROM settings_pogoauth WHERE device_id IS NULL AND account_id < 43;"
    )
...
def append_extra_data_to_accounts(availableAccounts):
    queryVariablePreparedUsernames = build_database_variable_from_accounts_list(
        availableAccounts
    )

    storedAvailableAccounts = get_data_from_database(
        f"SELECT username, nb_lures FROM account_lure WHERE username IN ({queryVariablePreparedUsernames}) ORDER BY nb_lures ASC;",
        "poliswag",
    )
    existingUsernames = [
        account["data"][0] for account in storedAvailableAccounts
    ]  # Extract usernames
    ...
    if len(availableAccounts) > 0:
        for account_data in storedAvailableAccounts:
            ...
            if nb_lures > 0:
                availableAccountsList.append(
                    {"username": username, "nb_lures": nb_lures}
                )

        for username in availableAccounts:
            if username not in existingUsernames:
                execute_query_to_database(
                    f"INSERT INTO account_lure (username) VALUES ('{username}');",
                    "poliswag",
                )
...
async def notify_accounts_available_message(message):
    ...
    for account in availableAccounts:
        tempString = (
            tempString + f"{account['username']} - {account['nb_lures']} lures\n"
        )

    await message.channel.send(
        embed=build_embed_object_title_description(
            "LISTA DE CONTAS DISPONÍVEIS", tempString
        )
    )
...
async def update_playintegrity_job(message):
    ...
    await message.channel.send(
        embed=build_embed_object_title_description(
            "PlayIntegrityFix command updated", playIntegrityCommand
        )
    )
