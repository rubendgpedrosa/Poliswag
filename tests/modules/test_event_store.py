...
class TestGetAllEventTypes:
    async def test_returns_db_rows_verbatim(self, event_store, db):
        ...
        db.get_data_from_database.return_value = rows
        assert await event_store.get_all_event_types() is rows
        sql = db.get_data_from_database.call_args.args[0]
...
class TestIsExcluded:
    async def test_returns_true_when_matching_row_found(self, event_store, db):
        db.get_data_from_database.return_value = [{"type": "community_day"}]
        assert await event_store.is_excluded("community_day") is True

    async def test_returns_false_when_no_rows(self, event_store, db):
        db.get_data_from_database.return_value = []
        assert await event_store.is_excluded("anything") is False

    async def test_uses_parametrised_query(self, event_store, db):
        db.get_data_from_database.return_value = []
        await event_store.is_excluded("community_day")
...
class TestAddExcluded:
    async def test_issues_insert_with_params(self, event_store, db):
        await event_store.add_excluded("raid_hour")
...
class TestRemoveExcluded:
    async def test_delegates_execute_return_value(self, event_store, db):
        db.execute_query_to_database.return_value = 1
        assert await event_store.remove_excluded("raid_hour") == 1
        call = db.execute_query_to_database.call_args
        assert "DELETE FROM excluded_event_type WHERE type = %s" in call.args[0]
        assert call.kwargs["params"] == ("raid_hour",)

    async def test_returns_zero_when_no_rows_affected(self, event_store, db):
        db.execute_query_to_database.return_value = 0
        assert await event_store.remove_excluded("nothing") == 0


class TestClearExcluded:
    async def test_returns_previous_count(self, event_store, db):
        db.get_data_from_database.return_value = [{"count": 3}]
        assert await event_store.clear_excluded() == 3
        delete_sql = db.execute_query_to_database.call_args.args[0]
        assert "DELETE FROM excluded_event_type" in delete_sql

    async def test_returns_zero_on_empty_table(self, event_store, db):
        db.get_data_from_database.return_value = []
        assert await event_store.clear_excluded() == 0
