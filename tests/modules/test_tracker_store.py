...
class TestExists:
    async def test_returns_true_when_row_present(self, tracker_store, db):
        ...
        assert await tracker_store.exists("pikachu") is True

    async def test_returns_false_when_no_rows(self, tracker_store, db):
        db.get_data_from_database.return_value = []
        assert await tracker_store.exists("missingno") is False

    async def test_passes_target_as_parametrised_query(self, tracker_store, db):
        db.get_data_from_database.return_value = []
        await tracker_store.exists("pikachu")
...
class TestAdd:
    async def test_insert_sql_and_params(self, tracker_store, db, mocker):
        fixed_now = datetime(2026, 4, 7, 12, 0, 0)
        mocker.patch("modules.tracker_store.datetime").now.return_value = fixed_now

        await tracker_store.add("charizard", "alice")
...
class TestRemove:
    async def test_delegates_rowcount_from_execute(self, tracker_store, db):
        db.execute_query_to_database.return_value = 1
        assert await tracker_store.remove("pikachu") == 1
        call = db.execute_query_to_database.call_args
        assert "DELETE FROM tracked_quest_reward WHERE target = %s" in call.args[0]
        assert call.kwargs["params"] == ("pikachu",)

    async def test_returns_zero_when_no_rows_affected(self, tracker_store, db):
        db.execute_query_to_database.return_value = 0
        assert await tracker_store.remove("ghost") == 0


class TestClear:
    async def test_returns_count_before_delete(self, tracker_store, db):
        db.get_data_from_database.return_value = [{"count": 5}]
        assert await tracker_store.clear() == 5
    ...
    async def test_empty_table_returns_zero(self, tracker_store, db):
        db.get_data_from_database.return_value = []
        assert await tracker_store.clear() == 0
